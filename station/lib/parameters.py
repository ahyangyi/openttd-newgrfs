...
for s, year in station_meta:
    if s == "E88A9CA":
        station_settings.append(Parameter(f"{s}_ENABLE_TEMPLATE", 0, booldict))
    station_settings.append(Parameter(f"{s}_ENABLE_MODULAR", 0, booldict))
    station_settings.append(Parameter(f"{s}_INTRODUCTION_YEAR", year, limits=(0, 9999)))
station_settings.append(Parameter("E88A9CP_ENABLE_MODULAR", 0, booldict))

platform_settings = []
for platform, enabled in [("concrete", True), ("brick", False)]:
    platform_settings.append(Parameter(f"PLATFORM_{platform.upper()}", (0 if enabled else 1), booldict))

shelter_settings = []
for shelter, enabled in [("shelter_1", False), ("shelter_2", True)]:
    shelter_settings.append(Parameter(f"SHELTER_{shelter.upper()}", (0 if enabled else 1), booldict))
