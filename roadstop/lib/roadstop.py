...
class ARoadStop(grf.SpriteGenerator):
    ...
    def get_sprites(self, g, sprites=None):
        ...
        # extra_props = {
        #    "name_id": g.strings.add(g.strings[f"STR_ROAD_STOP_{self.translation_name}"]).get_persistent_id(),
        #    "class_name_id": g.strings.add(g.strings[f"STR_ROAD_STOP_CLASS_{self.class_label_plain}"]).get_persistent_id(),
        # }
